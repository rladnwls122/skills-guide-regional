...
4년치 2과제 전부다. 2023·2024는 파이프라인 한 줄기, 2025·2026은 독립 소문제 4개다. → [지형도 2-1절](/exam/00-competition-map/)
...
```mermaid
flowchart LR
  SQS@{ icon: "logos:aws-sqs", form: "square", label: "order-queue<br/>메시지 수", pos: "b", h: 46, w: 46 }
  KEDA@{ icon: "k8s:keda", form: "square", label: "keda-operator", pos: "b", h: 46, w: 46 }
  SO["ScaledObject<br/>queueLength: 5"]
  DEP@{ icon: "k8s:deploy", form: "square", label: "order-processor", pos: "b", h: 46, w: 46 }
  POD@{ icon: "k8s:pod", form: "square", label: "Pod 1~10", pos: "b", h: 46, w: 46 }

  subgraph EKS["<span class='icon--logos icon--logos--aws-eks'></span> order-cluster 1.31"]
...
**왜 막히나.** 스크립트가 자기 Bastion을 지우면 이후 채점이 전부 불가하다. → [축 2, 6절](/axis/04-axis-bastion/)
...
**핵심 조각.** → [축 4, 4-5절](/axis/06-axis-data/)
...
**요구·핵심 조각.** → [축 4, 4-6절](/axis/06-axis-data/)
...
```mermaid
flowchart LR
  APIGW@{ icon: "logos:aws-api-gateway", form: "square", label: "wsi-api<br/>POST /api", pos: "b", h: 46, w: 46 }
  KDS@{ icon: "logos:aws-kinesis", form: "square", label: "wsi-data-stream<br/>샤드 1", pos: "b", h: 46, w: 46 }
  KDF@{ icon: "logos:aws-kinesis", form: "square", label: "wsi-delivery-stream", pos: "b", h: 46, w: 46 }
  S3@{ icon: "logos:aws-s3", form: "square", label: "data/raw/YYYY/MM/dd/HH/", pos: "b", h: 46, w: 46 }
  CRAWL@{ icon: "logos:aws-glue", form: "square", label: "wsi-glue-crawler", pos: "b", h: 46, w: 46 }
  JOB@{ icon: "logos:aws-glue", form: "square", label: "wsi-glue-job<br/>raw x ref 조인", pos: "b", h: 46, w: 46 }
  RES@{ icon: "logos:aws-s3", form: "square", label: "result/", pos: "b", h: 46, w: 46 }

  APIGW --> KDS --> KDF --> S3 --> CRAWL --> JOB --> RES
```

한 줄기라 앞이 막히면 뒤가 통째로 죽는다. 막히는 지점 순서대로:
