...
flowchart LR
  USER(("채점자 curl"))
  ALB@{ icon: "logos:aws-elb", form: "square", label: "ALB<br/>기본 동작 403", pos: "b", h: 46, w: 46 }
  TG["타깃 그룹<br/>/health 로 상태 확인"]
  EC2@{ icon: "logos:aws-ec2", form: "square", label: "ASG 인스턴스<br/>2 AZ", pos: "b", h: 46, w: 46 }
  SM@{ icon: "logos:aws-secrets-manager", form: "square", label: "worldpay-secret", pos: "b", h: 46, w: 46 }
  RDS@{ icon: "logos:aws-rds", form: "square", label: "Aurora MySQL", pos: "b", h: 46, w: 46 }
  CW@{ icon: "logos:aws-cloudwatch", form: "square", label: "worldpay-log-group", pos: "b", h: 46, w: 46 }

  USER -->|"POST /v1/users"| ALB
  ALB -->|"규칙에 걸린 경로만"| TG
  TG --> EC2
  EC2 -->|"기동 시 1회"| SM
  EC2 -->|"포트 3307"| RDS
...
**이 사슬이 통째로 살아야 채점 6-1~6-3 과 7-3 이 붙는다.** 한 칸이라도 끊기면 4.5점이 함께 죽는다.
...
4년 내내 나온 항목이다. 앞의 모든 게 살아 있어야 통과한다. → [채점 해부 6절](/exam/02-scoring/)
...
<Quiz title="ALB 기본 동작과 규칙">
2026 요구를 만족하는 리스너 구성은?

- [x] 기본 동작 = fixed-response 403, 규칙 1 = `/v1/users`·`/health` 를 타깃 그룹으로 forward
- [ ] 기본 동작 = forward, 규칙 1 = 나머지 경로를 403 fixed-response
  > 채점 5-3 이 `Listeners[].DefaultActions[].FixedResponseConfig.StatusCode` 를 본다. 기본 동작이 forward 면 이 값이 없어 0점이다.
- [ ] 기본 동작 = forward, 앱이 404 대신 403 을 반환하도록 구현
  > 지급 바이너리는 수정할 수 없고, 채점은 ALB 설정을 직접 읽는다.
- [ ] 기본 동작 = redirect 403
  > redirect 는 상태 코드 3xx 다. fixed-response 여야 한다.

**기본 동작 자체가 403 이어야 한다.** 동작이 같아 보여도 채점이 읽는 필드가 다르다.
</Quiz>

<Quiz title="ASG 인스턴스 Name 태그">
`worldpay-app` Name 태그가 인스턴스에 붙게 하는 방법을 **모두** 고르시오.

- [x] Launch Template 의 태그 스펙(`instance` 리소스 타입)에 넣는다
- [x] ASG 태그에 `PropagateAtLaunch=true` 로 넣는다
- [ ] ASG 이름을 `worldpay-app` 으로 짓는다
  > ASG 이름은 `worldpay-asg` 여야 하고(채점 2-1), 인스턴스 태그와 별개다.
- [ ] Launch Template 이름에 넣는다
  > 템플릿 이름은 인스턴스에 전파되지 않는다.

**채점 2-3 은 `Reservations[].Instances[].Tags` 를 본다.** ASG 이름만 맞고 인스턴스에 태그가 없으면 1.5점을 잃는다.
</Quiz>
