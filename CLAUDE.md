...
지방기능경기대회 클라우드컴퓨팅 훈련 가이드를 담은 Astro Starlight 문서 사이트. 애플리케이션 코드는 없고 `.md` 콘텐츠가 전부다 — 하는 일은 **문서 작성**과 **사이트 설정 조정** 둘 중 하나다.
...
- **콘텐츠는 `src/content/docs/` 다.** 사이드바는 `guide/` 디렉터리를 autogenerate 하고 순서는 각 문서 frontmatter 의 `sidebar.order` 가 정한다.
- **문서는 `.md` 로 쓴다.** 본문에 `<선수 비번호>`, `{"name": "..."}` 같은 꺾쇠·중괄호가 그대로 나온다 — `.mdx` 면 JSX 로 파싱돼 빌드가 깨진다. 컴포넌트를 쓰는 랜딩(`index.mdx`)만 예외다.
- **사이트 내부 링크는 절대 경로로 쓴다**(`/guide/02-scoring/`). 상대 링크는 검증기가 잡지 않는다.
...
`starlight-links-validator` 가 죽은 내부 링크에서 빌드를 깨뜨린다. 문서를 고쳤으면 이걸 돌린다.
