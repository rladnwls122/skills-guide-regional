  ...
  integrations: [
    ...
    starlight({
      ...
      /* 안 주면 Starlight 가 /favicon.svg 를 가리키는데 그 파일이 없어 페이지마다
         404 가 하나씩 났다. 나침반에서 구운 180px PNG 다 — 헤더 로고(대회 로고)와
         일부러 다른 그림이다. 탭 아이콘은 전국판과 나란히 떴을 때 어느 쪽인지
         구분되어야 하고, 헤더는 사이트에 들어온 뒤라 제목 글자가 그 일을 한다. */
      favicon: '/favicon.png',
      /* 헤더 제목 왼쪽 로고. 전국판과 같은 대회 로고이고 파일도 같은 것을 쓴다 —
         두 문서가 한 계열임을 헤더에서 보여 준다. 구분은 옆의 제목 글자가 한다.
         원본이 아니라 256px 로 구운 것이다: Starlight 는 로고를 줄이지 않고 그대로
         내보내므로, 원본을 걸면 1.5rem 로 그려질 그림에 1MB 를 매 페이지 받는다.
         replacesTitle 은 켜지 않는다 — 그림만 남기면 어느 판인지 알 수가 없다. */
      logo: {
